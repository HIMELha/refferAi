...
<body>
    <div class="container m-auto bg-white h-screen">
        ...
        <section class="flex justify-between  gap-4 bg-gradient-to-tr from-red-100 to-cyan-200 w-full max-h-fit min-h-full p-2 pl-4 pr-4 ">
            ...
            <div class="earnings w-[900px] h-fit  p-4 bg-gradient-to-tr from-gray-600 to-blue-400 ">
                <div class="p-4">
                     <p class="text-[20px] py-2 text-center pl-4 text-white bg-red-400">withdrawals requests</p>
...
?>
                         <div class="tab flex w-full justify-center bg-white border-1 border-blue-500 p-2 ">
                            <div class="all pt-2">
                                <a href="withdrawals.php" class="tabs border-2 border-blue-400 p-2 px-5 bg-sky-500 text-gray-600 hover:text-white">ALL</a>
                            </div>
                ...
                <div class="tab-contents records w-full p-2 bg-white">
                    <?php
                        $sql1 = "SELECT * FROM withdrawals ORDER BY draw_date DESC ";
                        $run1 = mysqli_query($db, $sql1);
                        if(mysqli_num_rows($run1) > 0){
                            while($row1 = mysqli_fetch_assoc($run1)){    
                    ?>
                    <div class="flex justify-between items-center w-full mt-2 p-2 px-3 bg-gray-600 text-sm">
                        ...
                        if($row1['pay_condition'] == 'Processing'){
                            ?>
                        <div class="flex">
                            <a href="paided.php?id=<?php echo $row1['wid'];?>" class="text-green-700 process bg-gray-100 px-2 ml-2 p-1">Pay success</a>
                            <a href="rejected.php?id=<?php echo $row1['wid'];?>" class="text-red-500 process bg-gray-100 px-2 ml-2 p-1">Reject</a>
                        </div>
                        <?php
